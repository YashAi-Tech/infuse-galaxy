...
import { LANE_WIDTH, CUSTOM_MAPS } from '../../types';

const StarField: React.FC = () => {
  const speed = useStore(state => state.speed);
  const currentMapId = useStore(state => state.currentMapId);
...
const LaneGuides: React.FC = () => {
    const { laneCount, currentMapId } = useStore();
    const map = CUSTOM_MAPS.find(m => m.id === currentMapId) || CUSTOM_MAPS[0];
...
const RetroSun: React.FC = () => {
    ...
    const uniforms = useMemo(() => ({
        uTime: { value: 0 },
        uColorTop: { value: new THREE.Color(map.sunColorTop) },
        uColorBottom: { value: new THREE.Color(map.sunColorBottom) }
    }), []);

    useEffect(() => {
        if (matRef.current) {
            matRef.current.uniforms.uColorTop.value.set(map.sunColorTop);
            matRef.current.uniforms.uColorBottom.value.set(map.sunColorBottom);
        }
    }, [map.sunColorTop, map.sunColorBottom]);
...
const MovingGrid: React.FC = () => {
    const { speed, currentMapId } = useStore();
    const map = CUSTOM_MAPS.find(m => m.id === currentMapId) || CUSTOM_MAPS[0];
...
export const Environment: React.FC = () => {
  ...
  return (
    <>
      <color attach="background" args={[map.bgColor]} />
      <fog attach="fog" args={[map.bgColor, 40, 160]} />
      
      <ambientLight intensity={0.2} color={map.ambientColor} />
      <directionalLight position={[0, 20, -10]} intensity={1.5} color={map.dirLightColor} />
      <pointLight position={[0, 25, -150]} intensity={2} color={map.pointLightColor} distance={200} decay={2} />
